...
git's normal 3-way merge.

When resolving a conflict:

- Keep both entries for distinct URLs.
- Choose one complete version when both sides changed the same URL.
- Keep one entry-opening line per URL. The duplicate check recognizes those
  lines; duplicate entries sharing a physical line can silently keep the last
  entry. Repeated fields within an entry also use the last value.
- Rerun the check to normalize the resolved file.

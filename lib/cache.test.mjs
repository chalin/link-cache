...
test('parseOwned defaults a missing when to the run time, manual entries only', () => {
  ...
  for (const via of ['lychee', 'browser']) {
    // Tool-written entries always carry when; a missing one is damage to
    // surface, not a claim to date.
    assert.throws(
...
test('parseOwned requires a nonblank via', () => {
  // A blank via would count as provenance-bearing on merge-back while naming
  // no resolver.
  assert.throws(
    () =>
      parseOwned(
...
test('projectToCsv projects fresh for any entry with expires, lapsed included', () => {
  // Only prune reads lapse. Projecting the real ts instead would let a seed
  // older than max_cache_age re-check in PR lanes until the next prune.
...
test('mergeBack leaves a lapsed seed untouched on an echoed projection', () => {
  // Any expires projects fresh, lapsed included; when lychee serves the row it
  // echoes it back byte-exactly. Without echo detection the `expired` branch
  // would replace the seed wholesale (via lychee, comments dropped) though no
  // live re-check happened.
  const owned = {
    entries: [
      entry('https://a.example/', 206, 100, 'manual', {
        expires: '2026-01-01',
...
test('mergeBack replaces a lapsed seed re-checked live with the lychee result', () => {
  // Only a forwarded cache-age override gets lychee to re-check a projected
  // row. The override is spent; a live verdict (even an equal one) makes it a
  // plain lychee entry rather than a seed with a stale expiry. The projection
  // was fresh (NOW); the re-check landed later in the run, so its ts differs.
  const owned = {
    entries: [
      entry('https://a.example/', 206, 100, 'manual', {
        expires: '2026-01-01',
...
test('mergeBack records an expired entry whose re-check failed with the failure word', () => {
  // The stale seeded status must not survive a reported failure.
  const owned = {
    entries: [
      entry('https://a.example/', 206, 100, 'manual', {
        expires: '2026-01-01',
...
test('mergeBack keeps a holding expiry on a repeated failure', () => {
  // Failure words never project, so a repeated failure is a fresh report on
  // every run; rebuilding the entry must not shed a field the owner set.
  const owned = {
    entries: [

...
test('parseOwned resolves +Nd expiry sugar to a date from the run time', () => {
  // NOW is 2026-09-21T14:13:20Z; the sugar counts whole days from the
  // normalizing run, so the written date is what the author meant by "N days
  // from now" and is fixed on the first rewrite.
...
test('parseOwned defaults a missing when to the run time', () => {
  const text = `{\n  "https://a.example/": { "result": 200, "via": "manual" },\n}\n`;
  const { entries } = parseOwned(text, { now: NOW + 0.7 });
...
test('hasLapsed reads an inclusive end-of-day cutoff and exempts never', () => {
  // NOW is 2026-09-21T14:13:20Z.
  assert.equal(hasLapsed(undefined, NOW), false, 'no expiry never lapses');
  assert.equal(hasLapsed('never', NOW), false, 'never never lapses');
  assert.equal(hasLapsed('2026-09-21', NOW), false, 'today still holds');
  assert.equal(hasLapsed('2026-09-20', NOW), true, 'yesterday has lapsed');
});
...
test('projectToCsv projects real timestamps for entries without expires', () => {
  // One rule, every lane: lychee's max_cache_age governs an entry unless an
  // expires override says otherwise. The real ts is what lets it govern.
...
test('projectToCsv projects the real ts once expires has lapsed', () => {
  // A lapsed override is spent: the entry is back under lychee's regime until
  // prune drops it. Omitting the row would re-check it in PR lanes (R1).
  ...
  assert.equal(p.ts, 100, 'a lapsed expiry projects like no expiry');
});
...
test('mergeBack ignores an echoed fresh projection ts (no false recency)', () => {
  // An entry whose expires holds projects a fresh ts, and a cache hit echoes
  // it back into the CSV byte-exactly (verified against lychee 0.24.2).
  // Adopting it would freshen `when` on every run, erasing the entry's real
  // age. The projection map identifies the echo; a genuinely re-checked URL
  // carries a different ts and still adopts.
  const owned = {
...
test('mergeBack leaves a lapsed seed untouched on an echoed projection', () => {
  // A lapsed expiry projects the real ts; when lychee serves the row it echoes
  // it back byte-exactly. Without echo detection the `expired` branch would
  ...
  const merged = mergeBack(
    owned,
    [{ url: 'https://a.example/', status: 206, ts: 100 }],
    { now: NOW, projectedTs: new Map([['https://a.example/', 100]]) },
  );
...
test('mergeBack replaces a lapsed seed re-checked live with the lychee result', () => {
  // The override is spent; a live verdict (even an equal one) makes it a
  // plain lychee entry rather than a seed with a stale expiry.
  const owned = {
    entries: [
      entry('https://a.example/', 206, 100, 'manual', {
        expires: '2026-01-01',
  ...
  assert.equal(a.ts, NOW, 'the live check time is recorded');
...
test('mergeBack keeps a holding expiry through a lychee re-confirmation', () => {
  // An expires on a lychee-owned entry must survive the ts refresh, or the
  // override would silently vanish on the first refresh-lane re-check.
  const owned = {
    entries: [

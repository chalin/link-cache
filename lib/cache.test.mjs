...
test('parseOwned reads a legacy status field as result (one-version migration)', () => {
  ...
  assert.throws(
    () => parseOwned(legacy(0)),
    /malformed entry/,
    'status 0 (unchecked) was never written and has no result mapping',
  );
  assert.throws(
    () =>
      parseOwned(
...
test('projectToCsv keeps only HTTP results', () => {
  ...
  assert.deepEqual(
    ...
    ['https://ok.example/'],
    'failure-word entries stay owned-file-only',
  );
});

test('projectToCsv defaults to fresh timestamps for every entry', () => {
  // Staleness checks off by default: every cached entry projects with a fresh
  // ts, so lychee's max_cache_age never bites and expired seeds are not
  // re-checked — PR runs verify only URLs not in the cache.
  const projected = projectToCsv(
    [
...
test('mergeBack leaves an expired seed untouched on an echoed projection', () => {
  ...
  // re-check happened — the round-16 overwrite hazard, now on every PR run.
  const owned = {
    entries: [
      entry('https://a.example/', 206, 100, 'manual', {
        expires: '2026-01-01',

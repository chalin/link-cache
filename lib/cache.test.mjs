...
test('parseOwned resolves +Nd expiry sugar to a date from the run time', () => {
  ...
  for (const [sugar, date, lapsed] of [
    ['+0d', '2026-09-21', true],
    ['+1d', '2026-09-22', false],
    ['+7d', '2026-09-28', false],
    ['+365d', '2027-09-21', false],
  ]) {
...
test('mergeBack replaces a lapsed seed re-checked live with the lychee result', () => {
  // Input: a same-day expires on a named-resolver entry (lapsed under the
  // start-of-day cutoff only; lapse is via-regardless), re-checked later in the
  // run than the fresh projection.
  const owned = {
    entries: [

...
test('parseOwned rejects duplicate URL keys', () => {
  const dup = `{
  "https://a.example/": { "status": 200, "when": "1970-01-01T00:00:01Z", "via": "lychee" },
  "https://a.example/": { "status": 404, "when": "1970-01-01T00:00:02Z", "via": "lychee" },
}
`;
  assert.throws(
    () => parseOwned(dup),
...
test('parseOwned rejects statuses outside the HTTP and tool-error domains', () => {
  const withStatus = (s) =>
    `{\n  "https://a.example/": { "status": ${s}, "when": "1970-01-01T00:00:01Z", "via": "lychee" },\n}\n`;
  assert.throws(
    () => parseOwned(withStatus(1)),
    /malformed entry/,
    'a projected sub-100 status would wedge the lychee CSV load',
  );
  assert.throws(
    () => parseOwned(withStatus(1000)),
    /malformed entry/,
    'statuses above 999 are rejected',
  );
  for (const ok of [0, -40, 100, 999]) {
    assert.equal(
      parseOwned(withStatus(ok)).entries[0].status,
      ok,
      `status ${ok} is in-domain`,
    );
  }
});
...
test('parseCsvLine rejects empty and non-numeric fields', () => {
  ...
  // status-0 or epoch-0 entries instead of counting as malformed.
  for (const bad of [
    'https://a.example/,200,',
    'https://a.example/,,123',
...
test('mergeBack leaves an evidence-free CSV absence untouched', () => {
  ...
  assert.equal(b.status, 200, 'the entry keeps its status');
...
test('mergeBack records a reported failure as a tool-error status', () => {
  ...
  const merged = mergeBack(
    ...
    { now: NOW, failedUrls: new Set(['https://b.example/']) },
  );
  const b = merged.entries.find((e) => e.url === 'https://b.example/');
  assert.equal(b.status, STATUS_CLIENT_ERROR, 'failure recorded as tool error');
...
test('mergeBack records a failure for a URL new to the cache', () => {
  ...
  const merged = mergeBack({ entries: [], trailing: [] }, [], {
    now: NOW,
    failedUrls: new Set(['https://new-dead.example/']),
  });
  ...
  assert.equal(e.status, STATUS_CLIENT_ERROR, 'recorded as tool error');
  assert.equal(e.via, 'lychee', 'credited to the lychee run');
});

test('mergeBack mints new entries for http(s) failures only', () => {
  // A -40 never projects, so it heals only via a later CSV success row —
  // and lychee persists only http(s) rows, so a minted file:// or mailto:
  // entry would be a permanent zombie (file:// keyed by a machine-specific
  // absolute path, to boot). Owned entries are the owner's own keys and
  // still record failures whatever their scheme.
  const merged = mergeBack(
    {
      entries: [
        {
    ...
    [],
    {
      ...
      failedUrls: new Set([
        ...
        'file:///home/runner/site/missing.html',
        'mailto:new@dead.example',
        'mailto:owned@dead.example',
      ]),
    },
  );
  assert.deepEqual(
  ...
  assert.ok(
    merged.entries.every((e) => e.status === STATUS_CLIENT_ERROR),
...
test('mergeBack lets failure evidence beat a residual CSV row', () => {
  ...
  const merged = mergeBack(
    ...
    { now: NOW, failedUrls: new Set(['https://flaky.example/']) },
  );
...
test('mergeBack records an expired entry whose re-check failed as a tool error', () => {
  ...
  const merged = mergeBack(owned, [], {
    now: NOW,
    failedUrls: new Set(['https://a.example/']),
  });
  const a = merged.entries[0];

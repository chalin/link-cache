...
test('parseOwned resolves +Nd expiry sugar to a date from the run time', () => {
  // NOW is 2026-09-21T14:13:20Z; rationale at resolveExpires and expiryCutoff.
  const withSugar = (sugar) =>
    `{\n  "https://a.example/": { "result": 200, "when": "1970-01-01T00:00:01Z", "via": "manual", "expires": ${JSON.stringify(sugar)} },\n}\n`;
  const lateDay = Date.parse('2026-09-21T23:59:59Z') / 1000;
  for (const [sugar, now, date, lapsed] of [
    ['+0d', NOW, '2026-09-21', true],
    ['+1d', NOW, '2026-09-22', false],
    ['+7d', NOW, '2026-09-28', false],
    ['+365d', NOW, '2027-09-21', false],
    ['+0d', lateDay, '2026-09-21', true],
    ['+1d', lateDay, '2026-09-22', false], // holds one more second only
  ]) {
    const { entries } = parseOwned(withSugar(sugar), { now });
    assert.equal(entries[0].expires, date, `${sugar} resolves to ${date}`);
    assert.equal(
      hasLapsed(entries[0].expires, now),
      lapsed,
      `${sugar} is ${lapsed ? 'lapsed' : 'holding'} in the resolving run`,
    );
  }
});
...
test('hasLapsed lapses at the first instant of the expiry UTC day', () => {
  ...
  assert.equal(hasLapsed('2026-09-21', start - 0.001), false, 'the eve holds');
  assert.equal(hasLapsed('2026-09-21', start), true, 'midnight lapses');
  assert.equal(hasLapsed('2026-09-21', NOW), true, 'the day itself is lapsed');
});
...
test('mergeBack replaces a lapsed seed re-checked live with the lychee result', () => {
  // Input: a named-resolver entry (lapse is via-regardless) whose expires is
  // NOW's own date, lapsed under the start-of-day cutoff only, re-checked later
  // in the run than the fresh projection.
  const owned = {
    entries: [

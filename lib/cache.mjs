...
// with an optional "expires": "YYYY-MM-DD" | "never" on any entry. `result` is
...
// Rejects unknown fields (a rewrite would silently drop them). `when` and
// `expires` are optional; parseOwned fills and resolves them.
...
// leftover to paper over. `now` dates entries without `when` and resolves
// "+Nd" expiry sugar to a date, so hand-authored entries are written back
// canonical on the next rewrite.
...
// run. A holding `expires` projects a fresh ts so lychee serves the entry; a
// lapsed one still projects (real `when`): omitting the row would re-check
// the URL in PR lanes, and retiring it is prune's job.

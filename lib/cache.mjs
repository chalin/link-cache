...
export function parseOwned(text, { now = Date.now() / 1000 } = {}) {
  ...
  for (const [url, raw] of Object.entries(obj)) {
    ...
    // year 9999, which the next read would reject).
    const expiresOk =
      expires === undefined ||
      expires === EXPIRES_NEVER ||
      isCanonicalDate(expires);
...
//     word as its result: for existing entries (keeping their comments and
//     `expires`: the rationale still explains the URL, and prune reads the
//     expiry) whatever their scheme, and for URLs

...
// format contract (shape, fields, why entries are multi-line) is
// https://github.com/chalin/link-cache/blob/main/docs/cache-format.md; the
// tests lock it.
//
// `when` is validated strictly to the canonical whole-second UTC form
// (YYYY-MM-DDTHH:MM:SSZ): the tool is the only writer of tool-owned entries, so
// a non-canonical value is a bug to surface, not smooth over (the htmltest-era

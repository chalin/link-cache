...
// `trailing`. Duplicate URL keys follow JSON semantics (last wins), merging
// both comment runs.
export function parseOwned(text) {
  ...
  for (const raw of lines) {
    ...
    if (open && pending.length) {
      ...
      commentsByUrl.set(url, [...(commentsByUrl.get(url) ?? []), ...pending]);
      pending = [];
    }
  }
...
//     served its purpose);
//   - projected or expired entry missing from the CSV: Lychee dropped or
//     failed it (it never persists errors) — recorded as STATUS_CLIENT_ERROR
//     under via "lychee", keeping the entry's comments (the rationale still
//     explains the URL). Without this, an expired seed whose live re-check
//     failed would stay stale forever.
export function mergeBack(owned, csvEntries, { now = Date.now() / 1000 } = {}) {
  ...
  for (const entry of owned.entries) {
    ...
    if (!csv) {
      if (projectedUrls.has(entry.url) || expired) {
        merged.push({
          url: entry.url,
        ...
        merged.push(entry); // never projected (error status): untouched
      }
      continue;
    }

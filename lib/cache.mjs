...
//     and for URLs new to the cache alike, and beating a residual CSV row for
//     the same URL;
...
export function mergeBack(
  ...
  // dead link" case, where the run's own report is the only evidence.
  for (const url of failed) {
    merged.push(failureEntry(url));
  }

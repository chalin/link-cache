...
//     persists only http(s) rows), and file:// keys embed machine-specific
...
export function mergeBack(
  ...
  // Failures with no owned entry and no CSV row — the "new page introduces a
  // dead link" case, where the run's own report is the only evidence. Minting
  // is http(s)-only (see the rules above).

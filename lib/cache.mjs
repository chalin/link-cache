...
// format contract is
// https://github.com/chalin/link-cache/blob/main/docs/cache-format.md; the
// tests lock it.
...
// `when` is validated strictly to the canonical whole-second UTC form: the tool
// is the only writer of tool-owned entries, so a non-canonical value is a bug to
...
// Canonical `when` -> epoch seconds. Returns NaN for a non-canonical form or a
// date requiring normalization; throws RangeError for an unparseable date in
// the canonical form.
...
// Parse one URL,STATUS,TIMESTAMP line; null when malformed. STATUS permits an
// optional leading minus; TIMESTAMP is unsigned. Both require at least one
// decimal digit, and URL must be non-empty and satisfy the CSV quote grammar
// (csvUnquote).
...
// Work per line so commas inside URL strings are never touched.
...
// A result is an HTTP status (100-999) or a known failure word.
...
// optional on any entry. `when` may be omitted only on manual entries (a
// tool-written entry without it is damage to surface, not a claim to date).
...
// entry: projection ignores lapse. Projecting the real `when` after lapse would

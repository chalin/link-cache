...
// failure word. A legacy `status` field is accepted on read and mapped to
// `result` (never both): the 0.4.x negative codes become failure words, and
// status 0 (unchecked, never written) has no mapping.
...
function migrateLegacyStatus(v) {
  ...
  const result =
    Number.isInteger(status) && status < 0
      ...
      : status;
  return result === undefined ? v : { ...rest, result };
}
...
// Project the owned entries into Lychee's CSV. Only HTTP results project —
// Lychee never persists errors, and would re-drop the rest. By default every
// entry projects with a fresh timestamp (staleness checks not applied):
// lychee's max_cache_age never bites and expired seeds are not re-checked, so
// a run verifies only URLs not in the cache. With `checkStale`, real
// timestamps project so max_cache_age applies, and manual `expires` governs
// its entry: unexpired seeds still project fresh (the expiry date, not
// max_cache_age, owns a seed's lifecycle) while expired ones are omitted,
// forcing a live re-check.
export function projectToCsv(
  ...
  for (const e of entries) {
    if (typeof e.result !== 'number') continue;
    if (!checkStale) {
...
//     projected — `projectedTs`): the entry is untouched. Under the default
...
export function migrateCsvText(csvText) {
  ...
  const owned = {
    entries: entries.map(({ url, status, ts }) => ({
      url,
      result: status,
      ts,
      via: 'lychee',

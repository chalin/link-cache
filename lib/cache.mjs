...
// Lychee never persists errors and would re-drop the rest, so recorded
// failures re-check on every run. By default every projected entry carries a
// fresh timestamp (staleness checks not applied): lychee's max_cache_age
// never triggers and expired seeds are not re-checked, so a run verifies only
// URLs without a cached HTTP result. With `checkStale`, real timestamps
// project so max_cache_age applies, and manual `expires` governs its entry:
// unexpired seeds still project fresh (the expiry date, not max_cache_age,
// owns a seed's lifecycle) while expired ones are omitted, forcing a live
// re-check.

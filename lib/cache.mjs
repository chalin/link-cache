...
// entry; lapse is read by prune (which drops the entry) and by mergeBack (a
// live re-check spends it), never here. Projecting the real `when` after lapse
// would let a seed older than max_cache_age re-check in PR lanes until the
// next prune drops it.

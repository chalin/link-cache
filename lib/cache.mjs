...
// with an optional "expires": "YYYY-MM-DD" | "never" on any entry, the
// per-entry override of lychee's max_cache_age. Hand-authoring conveniences
// resolve on read and are written back canonical on the next rewrite: a
// missing `when` becomes the run's time, and the sugar "+Nd" becomes the
// date N days from the run. `result` is
...
export const EXPIRES_NEVER = 'never';
...
function resolveExpires(expires, now) {
  ...
  if (!m) return expires;
  return tsToWhen(Math.floor(now) + Number(m[1]) * DAY).slice(0, 10);
}

// An expiry date holds through the end of its UTC day: the cutoff is the
// following midnight, compared with >= so a fractional `now` inside the last
// second still holds.
const expiryCutoff = (isoDate) =>
  Date.parse(`${isoDate}T00:00:00Z`) / 1000 + DAY;

// True once a resolved `expires` date is behind `now`; absent and `never`
// never lapse.
...
// Rejects unknown fields (a rewrite would silently drop them). `when` may be
// absent on a hand-written entry (parseOwned dates it by the run); `expires`
// may sit on any entry: provenance and lifetime are orthogonal.
...
// leftover to paper over. `now` dates hand-written entries and resolves
// expiry sugar.
export function parseOwned(text, { now = Date.now() / 1000 } = {}) {
  ...
  for (const [url, raw] of Object.entries(obj)) {
    ...
    entries.push({
      ...
      via: v.via,
      expires: resolveExpires(v.expires, now),
...
// run. One rule for every lane: an entry projects its real timestamp, so
// lychee's max_cache_age governs it, unless an `expires` override holds (a
// future date, or `never`), in which case a fresh timestamp makes lychee
// serve it. A lapsed override is spent and projects like none: prune retires
// the entry; omitting the row here would re-check it in PR lanes instead.
export function projectToCsv(entries, { now = Date.now() / 1000 } = {}) {
  ...
  for (const e of entries) {
    ...
    const holds = e.expires !== undefined && !hasLapsed(e.expires, now);
    projected.push({ url: e.url, status: e.result, ts: holds ? fresh : e.ts });
  }
  return projected;
}
...
//     adopting it would freshen `when` on every served entry with a holding
//     `expires`, erasing real ages, and would let the `expired` branch
//     overwrite seeds no live check ever touched;
//   - entry with a lapsed `expires` re-checked live: replaced wholesale (the
//     override is spent; a live verdict makes it a plain lychee entry);

...
// newest timestamp. Equal-timestamp duplicates with different statuses are
// ambiguous (order would decide): both rows are dropped and counted as
// conflicting, so a one-shot consumer (migration) can refuse.
export function parseCsv(text) {
  ...
  for (const raw of text.split('\n')) {
    ...
    if (prior && entry.ts === prior.ts && entry.status !== prior.status) {
      conflicted.add(entry.url);
      continue;
    }
    if (!prior || entry.ts >= prior.ts) byUrl.set(entry.url, entry);
  }
  for (const url of conflicted) byUrl.delete(url);
  return {
    entries: [...byUrl.values()].sort(byUrlBytes),
    malformed,
...
// (the parser would reject the output: a false-clean migration). Ambiguous
// duplicate rows surface via `conflicting` (parseCsv).

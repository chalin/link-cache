...
- `min-release-age=7`: a version must have been on the registry for a week
  before npm will install it. Most malicious releases are pulled within days;
  the cooldown lets that happen before the version can reach us. Bumps therefore
  wait a week after upstream publishes.
- `ignore-scripts=true` and `strict-allow-scripts=true`: lifecycle scripts
  (`preinstall`, `postinstall`, ...) never run. No dependency here needs them.
  `install:safe` repeats `--ignore-scripts` explicitly so the control survives
  an `.npmrc` regression.
- `engine-strict=true`: the `engines` field is enforced, so an unsupported Node
  fails at install time rather than at first run.
- `script-shell=bash`: one interpreter for npm scripts on every platform (npm on
  Windows defaults to `cmd.exe`, whose quoting diverges silently).

## Pinned actions and a script-free publish

- Workflow actions in [`publish.yaml`][] are pinned to full commit SHAs, with
  the version in a trailing comment for readability; a tag can be moved, a SHA
  cannot.
- The publish job installs nothing and runs `npm publish --ignore-scripts`. The
  check workflow gates every push, so the release commit has already passed
  `npm run check`; re-running an install under the job that holds the OIDC
  `id-token` would only let registry-delivered code run with publish authority.
...
`npx BIN` on a machine whose `node_modules` is stale or missing falls back to
the public registry and runs whatever package holds that bin name there. The
`lychee-norm-cache` name was squatted this way in 2026, before the package
reached the registry. Consequences for this repo:

- Never wire bare `npx` in scripts, CI, or docs; use bare bin names in `npm run`
  scripts (`node_modules/.bin` is on the `PATH`, so they resolve locally or fail
  loudly).
